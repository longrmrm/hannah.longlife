...
public class StringUtil {
	...
	public static String columnToProperty(String columnName) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (c == '_')
				buff[count++] = Character.toUpperCase(columnName.charAt(++i));
			else
	...
	/**
	 * For example: initialRsrcConsumerGroup -> InitialRsrcConsumerGroup
	 * @param str
	 * @return
	 */
	...
	public static String getSubstring(String str, String prefix, String suffix) {
		return getSubstring(str, prefix, suffix, false);
	}

	/**
	 * get substring of str
	 * @param str
	 * @param prefix not included
	 * @param suffix not included
	 * @param isLast suffix is the rightmost or not
	 * @return
	 */
	public static String getSubstring(String str, String prefix, String suffix, boolean isLast) {
		if (!isNull(prefix)) {
			int index = str.indexOf(prefix);
			str = index == -1 ? "" : str.substring(index + prefix.length());
		}
		if (isNull(suffix))
			return str;
		else {
			int index = isLast ? str.lastIndexOf(suffix) : str.indexOf(suffix);
	...
	public static char randomChar(String type) {
		...
		char t = isNull(type) ? '*' : type.charAt(random.nextInt(type.length()));
		switch (t) {
		case 'u':
